#!/usr/bin/env python3
"""tools/tm_expense_import_wechat.py — P4 wechat CSV bill importer."""
...
def import_csv(csv_path: str, dry_run: bool = False) -> dict[str, Any]:
    """Parse wechat CSV bill and write via batch_record.
    ...
    # Find header row: wechat CSVs may have noise lines like "---微信支付账单明细---"
    header_idx = None
    ...
    if header_idx is None:
        return {"ok": False, "error": "no header row found in wechat CSV"}
    ...
    for row_idx, row in enumerate(reader):
        ...
        try:
            ...
            if occurred_at is None:
                skipped_invalid += 1
                errors.append({"row": header_idx + row_idx + 2, "error": f"invalid 交易时间: {time_str}"})
                continue
            ...
            # --- 交易单号 ---
            source_external_id = (row.get("交易单号") or "").strip() or None

            # --- 支付方式 ---
            pay_method_raw = (row.get("支付方式") or "").strip()
            payment_method = f"wechat:{pay_method_raw}" if pay_method_raw else "wechat"
            ...
            skipped_invalid += 1
            errors.append({"row": header_idx + row_idx + 2, "error": str(e)})
...
def _parse_datetime(s: str) -> str | None:
    """Parse wechat datetime formats to ISO 8601."""
    s = s.strip()
    formats = [
        "%Y-%m-%d %H:%M:%S",
        "%Y/%m/%d %H:%M:%S",
...
def _infer_category(txn_type: str, merchant: str | None, note: str | None) -> str:
    """Infer category from wechat transaction type / merchant / note."""
