...
DEV_CLOSEOUT_COMPLETION_MARKERS = (
    "本轮开发",
    "开发已经完成",
    "实现范围",
    "关键文件",
...
DEV_CLOSEOUT_VALIDATION_MARKERS = (
    "验证已跑",
    "pytest",
    "py_compile",
    "passed",
...
【开发收尾记录例外】
如果内容是开发收尾记录，并包含 commit / push、关键文件、验证命令与结果、运行态 warning 或剩余 blocker，
它是后续 agent 复盘和接手所需的持久系统事实。不要仅因出现“本轮开发”“当前 worktree”“Goal 完成”
就标为瞬态。只有没有 commit、没有文件、没有验证结果的泛泛进度播报才应视为一次性状态。
...
def _is_durable_development_closeout(text: str, topic: str) -> bool:
    """Return True for implementation closeout records with durable evidence."""
    if topic not in {"systems", "operations", "cross"}:
        return False
    return (
        _has_any_marker(text, DEV_CLOSEOUT_COMPLETION_MARKERS)
        and _has_any_marker(text, DEV_CLOSEOUT_VERSION_MARKERS)
        and _has_any_marker(text, DEV_CLOSEOUT_VALIDATION_MARKERS)
        and _has_any_marker(text, DEV_CLOSEOUT_ARTIFACT_MARKERS)
    )
...
def route_memory(
    ...
    is_durable_development_closeout = _is_durable_development_closeout(text, topic)
    ...
    if is_durable_development_closeout and score >= 70 and not needs_human_review:
        return RouteDecision(
            route="mem0", score=score, topic_inferred=topic_inferred,
            issues=issues,
            reasons=f"durable development closeout: {reasons}",
            is_transient=False, is_sensitive=is_sensitive,
            needs_human_review=needs_human_review,
        )

    if is_durable_development_closeout and score >= 30:
        return RouteDecision(
            route="inbox", score=score, topic_inferred=topic_inferred,
            issues=issues,
            reasons=f"durable development closeout needs review: {reasons}",
            is_transient=False, is_sensitive=is_sensitive,
            needs_human_review=needs_human_review,
        )
