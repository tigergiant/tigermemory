...
from tigermemory_search.embed import _content_hash, _embed_text, _iter_pages, _partition_of
...
def build(*, mode: str, dim: int = DIMENSION, force: bool = False, limit: int | None = None) -> dict:
    ...
    for abs_path, rel, title, aliases, body in _iter_pages("wiki"):
        seen.add(rel)
        text = _embed_text(rel, title, aliases, body)
    ...
    for batch_start in range(0, len(pending), BATCH_SIZE):
        ...
        for (rel, title, aliases, body, partition, _text), (dense, sparse_pairs) in zip(batch, results):
            ...
            entry: dict[str, Any] = {
                ...
                "aliases": aliases,
                "partition": partition,
                "hash": _content_hash(rel, title, aliases, body),
                "mtime": mtime,
                "dim": len(dense),
                "dense": dense,
